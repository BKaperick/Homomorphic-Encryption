...
First, let us proof a useful lemma.
\begin{lemma}
    Given $a,b\in\Z$, and $x,y\in\Z$ and the constraint $b\mod{x}\mod{y} = 0$, then the following equality holds:
    \[ a + b\mod{x}\mod{y} = a\mod{x}\mod{y}.\]
    \label{lem}
\end{lemma}
\begin{proof}
    Let $a,b,x,y\in\Z$ be arbitrary such that $b\mod{x}\mod{y} = 0$.  Then, $b$ can be decomposed (not necessarily distinct) into $b = cx + dy$ for some $c,d\in\Z$.
    Then,
    \begin{align*}
        a+b\mod{x}\mod{y} &= a+ cx+dy\mod{x}\mod{y}\\
        &= a+dy\mod{x}\mod{y}
    \end{align*}<++>
\end{proof}<++>
...
Now, applying the first step of decryption to $c$, we have
    \[c\mod{s} = m + 2\left( r + s\sum_{i=0}^n c_{\alpha_i}\right) - \round{\frac{m + 2r + 2\sum_{i=0}^nc_{\alpha_i}s}{x_0}}(x_0)\]
and
    \[c\mod{s} = m + 2r - \round{\frac{m + 2r + 2\sum_{i=0}^nc_{\alpha_i}s}{c_0s+r_0}}r_0\mod{s}.\]
